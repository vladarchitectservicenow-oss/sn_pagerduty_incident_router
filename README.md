# sn_pagerduty_incident_router
Author: Vladimir Kapustin

## Architecture
```mermaid
graph TD
    SN[ServiceNow Instance] -->|REST| sn_pagerduty_incident_router
    sn_pagerduty_incident_router -->|Store| DB[ServiceNow Tables]
    sn_pagerduty_incident_router -->|Output| Report[MD/JSON Reports]
```
## Installation
```bash
git clone https://github.com/vladarchitectservicenow-oss/sn_pagerduty_incident_router.git
cd sn_pagerduty_incident_router
python3 src/cli.py --sn-url https://dev.instance.com --help
```
## ROI Calculator
| Metric | Manual | With sn_pagerduty_incident_router |
|--------|--------|-------------|
...
# Run scan
POST /api/x_sn_pagerduty_incident_router/scan
```
## Security & Compliance
...
## Overview
sn_pagerduty_incident_router is a production-grade ServiceNow scoped application developed by Vladimir Kapustin under AGPL-3.0.

## Architecture
```mermaid
graph TD
    SN[ServiceNow Instance] -->|REST| sn_pagerduty_incident_router
    sn_pagerduty_incident_router -->|Store| DB[x_sn_pagerduty_incident_router_tables]
    sn_pagerduty_incident_router -->|Output| Report[Reports MD/JSON/CSV]
...
## ROI Analysis
| Metric | Manual Process | With sn_pagerduty_incident_router |
|--------|---------------|-------------|
...
See `Validation/TEST CASES/sn_pagerduty_incident_router/test_suite_SOP.md`
...
## Support
- GitHub Issues: https://github.com/vladarchitectservicenow-oss/sn_pagerduty_incident_router/issues
- ServiceNow Community: Tag `sn_pagerduty_incident_router`
...
sn_pagerduty_incident_router is packaged using the ServiceNow Application Repository and Studio standards. It ships with a complete `sys_app` definition, scoped tables, Business Rules, Script Includes, Client Scripts, UI Policies, UI Actions, scheduled jobs, and inbound REST APIs. The application follows ServiceNow naming conventions: the scope is `x_sn_pagerduty_incident_router`, and all artifacts are prefixed consistently to avoid collisions with other applications.

In large organizations, manual execution of the workflows handled by sn_pagerduty_incident_router typically consumes 40–120 hours per year per team. At a blended hourly rate of $85/hour, this translates to $3,400–$10,200 annually in labor cost, not including opportunity cost, human error, or compliance penalties. sn_pagerduty_incident_router reduces this workload to fewer than 5 hours per year through automation, delivering an 87%+ cost reduction with immediate payback.
...
Multi-format reporting is a core competency of sn_pagerduty_incident_router. Rather than forcing teams into a single report format, the application supports Markdown (for GitHub and Confluence), JSON (for downstream ETL pipelines), and CSV (for Excel and BI ingestion). Reports are generated via a modular reporting engine that can be extended with plugins for PDF, HTML, or XLSX output.
...
4. Enter the repository URL: `https://github.com/vladarchitectservicenow-oss/sn_pagerduty_incident_router.git`.
...
|----------|------|---------|-------------|
| `x_sn_pagerduty_incident_router.logging.level` | Choice | INFO | DEBUG, INFO, WARN, ERROR |
| `x_sn_pagerduty_incident_router.scan.chunk_size` | Integer | 500 | Records per batch for large tables |
| `x_sn_pagerduty_incident_router.scan.delta.enabled` | Boolean | true | Enable incremental scanning |
| `x_sn_pagerduty_incident_router.output.format.default` | Choice | md | Default report format |
| `x_sn_pagerduty_incident_router.api.timeout_seconds` | Integer | 30 | REST call timeout |
| `x_sn_pagerduty_incident_router.encryption.key_id` | String | — | Reference to sys_encryption_context |
| `x_sn_pagerduty_incident_router.healthcheck.enabled` | Boolean | true | Enable health endpoint |

Roles:
- `x_sn_pagerduty_incident_router.admin`: Full configuration, deployment, and report access.
- `x_sn_pagerduty_incident_router.user`: Read-only report viewing and health-check access.
- `x_sn_pagerduty_incident_router.api`: Service account role for CI/CD pipelines.
...
| Memory exhaustion | Chunk size too large for JVM/Python heap | Reduce `x_sn_pagerduty_incident_router.scan.chunk_size` | Tune heap per instance sizing |
...
## Support & Community
- **GitHub Issues:** https://github.com/vladarchitectservicenow-oss/sn_pagerduty_incident_router/issues
- **Discussions:** https://github.com/vladarchitectservicenow-oss/sn_pagerduty_incident_router/discussions
- **ServiceNow Community:** Tag `sn_pagerduty_incident_router` in posts.
...
- ACL tests confirm `x_sn_pagerduty_incident_router.user` cannot modify configuration.
